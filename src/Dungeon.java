	...
	public void goDownOneFloor() {
		...
		if (currentFloorIndex == floors.size()) {
			...
			if(b2Spawn == currentFloorIndex) {
					floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.B2));
			}
			if(b3Spawn == currentFloorIndex) {
					floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.B3));		
			}
		
			if(b4Spawn == currentFloorIndex) {
				floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.B4));	
			}
			
			if(b5Spawn == currentFloorIndex) {
				floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.B5));	
			}
			
			if(b6Spawn == currentFloorIndex) {
				floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.B6));	
			}
			...
			if(c2Spawn == currentFloorIndex) {
				floors.get(currentFloorIndex).addArmor(new Armor(ArmorType.C2));	
			}
